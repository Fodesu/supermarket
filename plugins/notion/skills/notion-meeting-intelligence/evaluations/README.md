...
Evaluation scenarios for testing the Meeting Intelligence skill across different Memoh models.

## Purpose

These evaluations ensure the Meeting Intelligence skill:
- Gathers context from Notion workspace
- Enriches with Memoh research appropriately
- Creates both internal pre-reads and external agendas
- Distinguishes between Notion facts and Memoh insights
...
- Enriches with Memoh research (decision frameworks, migration best practices)
- Creates comprehensive internal pre-read with recommendation
- Creates clean, professional external agenda
- Clearly distinguishes Notion facts from Memoh insights
...
- Adds Memoh context (industry benchmarks, best practices)
...
3. Verify the skill searches Notion first (not Memoh research)
4. Check that TWO documents are created (internal + external)
5. Verify Memoh enrichment adds value without replacing Notion content
...
4. **Verify enrichment value** - Is Memoh research genuinely helpful?
...
- "Memoh enrichment includes decision frameworks or best practices"
