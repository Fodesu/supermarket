...
describe('Skill Registry definitions', () => {
  test('parses sources and applies skill, package, declared and default OS precedence', () => {
    const definition = parseSkillRegistryDefinition({
      ...
      refresh_interval: '12h', retention: { catalog_revisions: 30 },
    ...
    expect(definition.retention).toEqual({ catalog_revisions: 30 })
  ...
  test('rejects unknown adapters, unsafe paths and malformed overrides', () => {
    ...
    expect(() => parseSkillRegistryDefinition({
      schema_version: '1', id: 'bad', name: 'Bad', adapter: 'skill_directory', source: { type: 'local', path: 'skills' },
      refresh_interval: '12h', retention: { catalog_revisions: 30 }, package_overrides: { mac: { os: ['darwin'] } },
    ...
    expect(() => parseSkillRegistryDefinition({
      ...
      source: { type: 'local', path: 'skills' }, refresh_interval: '12h', retention: { catalog_revisions: 30 },
    ...
    }).retention).toEqual({ catalog_revisions: 30 })
    expect(() => parseSkillRegistryDefinition({
      schema_version: '1', id: 'bad', name: 'Bad', adapter: 'skill_directory',
      source: { type: 'local', path: 'skills' }, refresh_interval: '12h', retention: { catalog_revisions: 0 },
    })).toThrow('catalog_revisions')
  })
})
