import type { CatalogSkill, SkillCategorySummary } from '../types/skill-registry'
...
export function searchCatalogSkills(all: CatalogSkill[], options: SkillCatalogSearchOptions = {}) {
  const matches = all.filter((skill) => {
    ...
    if (options.os && !skill.runtime_requirements.os.includes(options.os.toLowerCase() as any)) return false
